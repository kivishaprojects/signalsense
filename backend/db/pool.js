...
async function migrate() {
  ...
  if (existing.rows.length === 0) {
    ...
    if (cityId) {
      ...
      const junctions = [
        { code: 'BPL-001', name: 'MG Road × DB Mall Road', lat: 23.2340, lng: 77.4340 },
        { code: 'BPL-002', name: 'Arera Colony × Hoshangabad', lat: 23.2180, lng: 77.4210 },
        { code: 'BPL-003', name: 'New Market × Sultania Road', lat: 23.2420, lng: 77.4080 },
      ];
      for (const j of junctions) {
        const jr = await pool.query(
          "INSERT INTO junctions (city_id, code, name, location, latitude, longitude) VALUES ($1,$2,$3,$4,$5,$6) ON CONFLICT DO NOTHING RETURNING id",
          [cityId, j.code, j.name, j.name, j.lat, j.lng]
        );
        if (jr.rows[0]) {
          for (const arm of ['North','South','East','West']) {
            await pool.query(
              ...
              [jr.rows[0].id, `${j.code}-CAM-${arm[0]}`, arm, `Camera ${arm}`]
            );
          }
        }
      }
